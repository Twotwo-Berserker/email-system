...
public interface MailStatusMapper extends BaseMapper<MailStatus> {
     ...
     * 标记已读
     */
    @Update("UPDATE mail_status SET is_read = 1, read_time = NOW() WHERE mail_id = #{mailId} AND user_id = #{userId}")
    int markAsRead(@Param("mailId") Long mailId, @Param("userId") Long userId);

    /**
     * 软删除（移至垃圾箱）
     */
    @Update("UPDATE mail_status SET is_deleted = 1 WHERE mail_id = #{mailId} AND user_id = #{userId}")
    int softDelete(@Param("mailId") Long mailId, @Param("userId") Long userId);

    /**
     * 恢复删除
     */
    @Update("UPDATE mail_status SET is_deleted = 0 WHERE mail_id = #{mailId} AND user_id = #{userId}")
